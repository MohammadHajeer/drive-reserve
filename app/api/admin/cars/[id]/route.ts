...
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error("Admin car load error:", error);
    ...
    console.error("Admin car route error:", error);
...
export async function DELETE(
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error: {
            code: "CAR_DEACTIVATION_FAILED",
            message: "Unable to deactivate the car.",
          },
        },
        { status: 500 },
      );
    ...
    console.error("Deactivate car route error:", error);
